...
if [ $RELEASE != 'raring' ] && [ $RELEASE != 'saucy' ] && [ $RELEASE != 'trusty' ] && [ $RELEASE != 'wily' ] && [ $RELEASE != 'focal' ] ; then
  PACKAGES+=' nfs-common'
fi
if [ $RELEASE != 'stretch' ] && [ $RELEASE != 'focal' ] ; then
...
if [ $SALT = 1 ]; then
  if $(lxc-attach -n ${CONTAINER} -- which salt-minion &>/dev/null); then
    ...
    if [ $DISTRIBUTION = 'ubuntu' ]; then
       if [ $RELEASE = 'precise' ] || [ $RELEASE = 'trusty' ] || [ $RELEASE = 'xenial' ]  || [ $RELEASE = 'focal' ] ; then
        ...
        if [ $RELEASE = 'precise' ]; then
          ...
          SALT_GPG_KEY="https://repo.saltstack.com/apt/ubuntu/16.04/amd64/latest/SALTSTACK-GPG-KEY.pub"
            ...
            SALT_SOURCE_1="http://ru.archive.ubuntu.com/ubuntu/ focal main"
            SALT_GPG_KEY="http://ru.archive.ubuntu.com/ubuntu/dists/focal/Release.gpg"
        fi
